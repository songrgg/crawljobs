...
class NeituiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_link = "http://www.neitui.me/neitui/page=%d.html" % (self.cur_page+1)
    ...
    def parse_detail(self, response):
        ...
        city              = response.xpath('//span[@class="jobtitle-r"]/text()').extract()[0] # city
        companyLogo       = response.xpath('//div[@class="img"]/a/img/@src').extract()[0] # company logo
        companyName       = response.xpath('//span[@class="jobtitle-l"]/text()').extract()[0] # company name

        company_panel = response.xpath('//dl[@class="ci_body"]/dd/text()').extract()
        companySize       = company_panel[1] # company size
        industryField     = company_panel[2] # industry field
        financeStage      = company_panel[3]
        website           = response.xpath('//a[@class="defaulta bluea"]/@href').extract()[0] # official website
        ...
        advantages = response.xpath('//li[@class="company_tag"]/span[@class="content"]/text()').extract()
        positionAdvantage = "[]"
        if len(advantages) > 0:
            positionAdvantage = '["%s"]' % string.join(advantages, '","') # position benefits

        positionFirstType = ''
        jobDescription    = response.xpath('//div[@class="jobdetail nooverflow"]').extract()[0] # job description
        workTime          = 8 # work period

        # requirement part
        workYear          = response.xpath('//span[@class="padding-r10 experience"]/text()').extract()[0] # work year
